        ...
        <div class="bread-box cms-left">
          ...
          <ul class="type-bread">
           ...
           <div class="cms-left channel-group">
              <el-button type="primary" @click="addChannel(1)">
      ...
      <el-table :data="tableData" stripe @selection-change="checkIdsAndStatus"  v-loading="loading">
        ...
        <el-table-column prop="title" label="标题" align="left" min-width="250px">
          <div slot-scope="scope" >
            ...
            <!-- <a class="link" :href="generateContentUrl(scope.row)" :title="scope.row.title" target="_blank">{{scope.row.title}}</a> -->
              <a class="link" :href="generateContentUrl(scope.row)" :title="scope.row.title" target="_blank">{{scope.row.title}}</a>
          </div>
        </el-table-column>
...
export default {
  ...
  methods: {
    ...
    generateContentUrl(data){
      if(data.link!=""){
        return data.link
      }else{
        var url = this.$store.state.sys.frontUrl+"/#/dpNewsDetail?channelId"+data.channelId+"&contentId="+data.id;
        return url;
      }
    }
  },
